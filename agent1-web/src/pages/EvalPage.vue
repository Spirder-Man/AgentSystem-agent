...
<template>
  <div class="space-y-6 max-w-5xl">
    ...
    <div class="bg-white border border-slate-200 rounded p-4">
      ...
      <div v-if="taskId" class="mt-4 p-3 bg-slate-50 border border-slate-200 rounded">
        <div class="flex items-center justify-between text-xs mb-2">
          ...
          <span :class="status === 'completed' ? 'text-green-600' : status === 'failed' ? 'text-red-600' : 'text-blue-600'" class="font-medium">{{ status }}</span>
        </div>
    ...
    <!-- 评测结果 -->
    <div v-if="report" class="bg-white border border-slate-200 rounded p-5 space-y-5">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-5 gap-3 text-xs">
        ...
        <div class="bg-slate-50 rounded p-3 text-center">
          ...
          <div :class="report.toolCallRate >= 0.8 ? 'text-green-600' : 'text-red-600'" class="font-bold text-lg">{{ formatPct(report.toolCallRate) }}</div>
        </div>
        <div class="bg-slate-50 rounded p-3 text-center">
          <div class="text-slate-400 mb-1">参数准确率</div>
          <div :class="report.parameterAccuracy >= 0.8 ? 'text-green-600' : 'text-red-600'" class="font-bold text-lg">{{ formatPct(report.parameterAccuracy) }}</div>
        </div>
        <div class="bg-slate-50 rounded p-3 text-center">
      ...
      <details class="text-xs">
        ...
        <div class="mt-3 space-y-2 max-h-96 overflow-y-auto">
          ...
          >
            ...
            <div class="text-[11px] text-slate-500">期望: {{ c.expectedTools.join(', ') }} → 实际: {{ c.actualTools.join(', ') || '无' }}</div>
          </div>
        </div>
      </details>
    </div>

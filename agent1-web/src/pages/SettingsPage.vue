...
async function sendTestAlert() {
  ...
  try {
    ...
    alertResult.value = { sent: false, recipient: '' };
  } finally {
  ...
  <div class="space-y-6 max-w-2xl">
    ...
    <div class="bg-white border border-slate-200 rounded p-4">
      ...
      <div v-else-if="health" class="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
        ...
        <div class="bg-slate-50 rounded p-2">
          ...
          <p :class="health.checks.database === 'connected' ? 'text-green-600' : 'text-red-600'">
            {{ health.checks.database }}
          </p>
        </div>
        <div class="bg-slate-50 rounded p-2">
          <span class="text-slate-400">LLM</span>
          <p :class="health.checks.ollama === 'reachable' ? 'text-green-600' : 'text-red-600'">
    ...
    <div class="bg-white border border-slate-200 rounded p-4 space-y-3">
      ...
      <div class="flex flex-wrap gap-3">
        <button
          @click="clearCache"
          class="text-xs px-3 py-1.5 rounded border border-amber-200 text-amber-700 bg-amber-50 hover:bg-amber-100"
        >
          🗑 清除查询缓存
        </button>
        <button
          @click="updateKB"
          class="text-xs px-3 py-1.5 rounded border border-blue-200 text-blue-700 bg-blue-50 hover:bg-blue-100"
        >
          📚 增量更新知识库
        </button>
      </div>
    ...
    <div class="bg-white border border-slate-200 rounded p-4">
      ...
      <button
        ...
        class="text-xs px-4 py-2 rounded border border-amber-200 text-amber-700 bg-amber-50 hover:bg-amber-100 disabled:opacity-50 transition-colors"
      >
      ...
      <div
        ...
        "
      >
        {{ alertResult.sent ? '✅ 邮件已发送至 ' + (alertResult.recipient || '未配置') : '❌ 发送失败' }}
      </div>
    </div>
    ...
    <div class="bg-white border border-slate-200 rounded p-4">
      ...
      <div class="text-xs text-slate-500 space-y-1">
        <p>
          用户: <span class="text-slate-700">{{ auth.username }}</span>
        </p>
        <p>
          角色: <span class="text-slate-700">{{ auth.role }}</span>
        </p>
        <p>
          Token: <span class="font-mono text-slate-400 text-[10px]">{{ auth.token?.slice(0, 30) }}…</span>
        </p>
      </div>
    </div>
  </div>

...
export const dashboardApi = {
  ...
  assets: () =>
    get<DashboardAssetsResponse>('/api/Dashboard/assets'),

  /** 自动合规扫描 —— 对全部资产执行 AI 合规检查（需 Auditor+ 角色） */
  scan: () =>
    post<DashboardScanResult>('/api/Dashboard/scan'),
  ...
  /** 历史巡检记录 —— 巡检计划及其关联轮次记录 */
  history: () =>
    get<DashboardHistoryResponse>('/api/Dashboard/history'),

  /** 安全隐患报告 —— 所有未关闭合规发现，按严重级别排序 */
  hazardReport: () =>
    get<DashboardHazardReport>('/api/Dashboard/report/hazard'),
};

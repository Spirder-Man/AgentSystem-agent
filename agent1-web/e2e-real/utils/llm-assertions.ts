...
export async function expectGbNumberPresent(
 ...
 * 验证工具调用链中包含指定工具名
 */
export async function expectToolCallPresent(
  page: Page,
  toolName: string,
  description = '工具调用链',
) {
  const toolChain = page.locator('[data-testid="tool-call-chain"]').or(
    page.locator(`text=${toolName}`),
  );
...
export function expectValidComplianceConclusion(response: unknown) {
  ...
  expect(typeof r.is_compliant === 'boolean' || r.is_compliant === undefined,
    'is_compliant 应为布尔值或未定义').toBeTruthy();
}

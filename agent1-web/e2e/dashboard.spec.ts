...
test.describe('P3: 仪表盘 — 合规总览 + 指标验证', () => {
  ...
  test('仪表盘应展示合规总览卡片和核心指标', async ({ page }) => {
    // 验证仪表盘标题
    const title = page.locator('text=合规总览').or(
      page.locator('text=总览仪表盘').or(page.locator('text=仪表盘'))
    );
  ...
  test('viewer 应能看到仪表盘但无扫描按钮', async ({ page }) => {
    ...
    // 仪表盘数据应可见
    const title = page.locator('text=合规总览').or(page.locator('text=总览仪表盘'));
    await expect(title.first()).toBeVisible({ timeout: 10_000 });
  ...
  test('仪表盘应展示不合规发现列表', async ({ page }) => {
    // 导航到发现列表（可能在仪表盘上或子页面）
    const findingsLink = page.locator('text=不合规发现').or(
      page.locator('text=合规发现').or(page.locator('text=Findings'))
    );
    ...
    if (await findingsLink.isVisible({ timeout: 5_000 }).catch(() => false)) {
      ...
      // 验证发现列表渲染
      const finding = page.locator('text=苯与丙酮').or(
        page.locator('text=Critical').or(page.locator('text=严重'))
      );
